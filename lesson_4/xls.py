...
def main():
    ...
    for row_number in range(1, sheet.max_row + 1):
        ...
        # buffer = sheet.cell(row=row_number, column=1).value
        if(clipbard_sentence == ""):
            print("Введите строку которую хотите добавить")
            stroka = input()
            sheet.cell(row=row_number, column=1).value = stroka
        else:
